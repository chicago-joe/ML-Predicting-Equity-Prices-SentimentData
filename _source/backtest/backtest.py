...
def SPY():
    ...
    spyprice = pd.read_csv(path + "SPY.csv")
    ...
    classret = [2 if next_Return[date] > sd_Return[date] * 1.0 else 1 if next_Return[date] > sd_Return[date] * 0.05 else 0 if next_Return[
                                                                                                                                  date] >
                                                                                                                              sd_Return[
                                                                                                                                  date] * -0.05 else -1 if
    next_Return[date] > sd_Return[date] * -1.0 else -2 for date in sd_Return.index]
    ...
    todayclassret = [
            2 if today_Return[date] > sd_Return[date] * 1.0 else 1 if today_Return[date] > sd_Return[date] * 0.05 else 0 if today_Return[
                                                                                                                                date] >
                                                                                                                            sd_Return[
                                                                                                                                date] * -0.05 else -1 if
            today_Return[date] > sd_Return[date] * -1.0 else -2 for date in sd_Return.index]
    # classret=[ 2  if ret>s2 else 1 if ret>s1 else 0 if ret>s0 else -1 if ret>sn1 else -2 for ret in next_Return]
...
def readdata():
    ...
    colum_names = ['ticker', 'date', 'description', 'sector', 'industry', 'raw_s', 's-volume', 's-dispersion', 'raw-s-delta',
                   'volume-delta', 'center-date', 'center-time', 'center-time-zone']
    df_2015 = pd.read_csv(path + 'SPY2015.txt', skiprows = 6, sep = '\t', names = colum_names)
    df_2016 = pd.read_csv(path + 'SPY2016.txt', skiprows = 6, sep = '\t', names = colum_names)
    df_2017 = pd.read_csv(path + 'SPY2017.txt', skiprows = 6, sep = '\t', names = colum_names)
    df_2018 = pd.read_csv(path + 'SPY2018.txt', skiprows = 6, sep = '\t', names = colum_names)
    df_2019 = pd.read_csv(path + 'SPY2019.txt', skiprows = 6, sep = '\t', names = colum_names)
    ...
    df = df.drop(
        columns = ['ticker', 'date', 'description', 'center-date', 'center-time', 'center-time-zone', 'raw-s-delta', 'volume-delta'])
    ...
    dffinal['volume_base_s_z'] = (dffinal['mean_volume_base_s'] - dffinal['mean_volume_base_s'].rolling(26).mean()) / dffinal[
        'mean_volume_base_s'].rolling(26).std()
    dffinal['s_dispersion_z'] = (dffinal['mean_s_dispersion'] - dffinal['mean_s_dispersion'].rolling(26).mean()) / dffinal[
        'mean_s_dispersion'].rolling(26).std()
    ...
    colum_names = ['ticker', 'date', 'description', 'sector', 'industry', 'raw_s', 's-volume', 's-dispersion', 'raw-s-delta',
                   'volume-delta', 'center-date', 'center-time', 'center-time-zone']
    df_2015 = pd.read_csv(path + 'ES_F2015.txt', skiprows = 6, sep = '\t', names = colum_names)
    df_2016 = pd.read_csv(path + 'ES_F2016.txt', skiprows = 6, sep = '\t', names = colum_names)
    df_2017 = pd.read_csv(path + 'ES_F2017.txt', skiprows = 6, sep = '\t', names = colum_names)
    df_2018 = pd.read_csv(path + 'ES_F2018.txt', skiprows = 6, sep = '\t', names = colum_names)
    df_2019 = pd.read_csv(path + 'ES_F2019.txt', skiprows = 6, sep = '\t', names = colum_names)
    df_temp = df_2015.append(df_2016, ignore_index = True)
    df_temp = df_temp.append(df_2017, ignore_index = True)
    df_temp = df_temp.append(df_2018, ignore_index = True)
    df_aggregate = df_temp.append(df_2019, ignore_index = True)
    ...
    testdf = pd.concat([testdf, next_Return, today_Return, classret, sd_Return, todayclassret], axis = 1, sort = False,
                       join = 'inner').dropna()
    return testdf
