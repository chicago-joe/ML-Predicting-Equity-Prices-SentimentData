...
def fnComputeVIXTerm(startDate=None, endDate=None):
    ...
    df['Norm_VIX-VIX3M'] =  np.log(abs(df['VIX-VIX3M']))
    df['Norm_VIX-VIX9D'] = np.log(abs(df['VIX-VIX9D'] ))
    df['Norm_VIX9D-VIX3M'] = np.log(abs(df['VIX9D-VIX3M']))

    # attach +/- to stationary signal
    # df[df.columns[-3]]=np.where(df[df.columns[4]]<0,df[df.columns[-3]]*-1,df[df.columns[-3]]*1)
    # df[df.columns[-2]]=np.where(df[df.columns[5]]<0,df[df.columns[-2]]*-1,df[df.columns[-2]]*1)
    # df[df.columns[-1]]=np.where(df[df.columns[6]]<0,df[df.columns[-1]]*-1,df[df.columns[-1]]*1)
...
# --------------------------------------------------------------------------------------------------
    ...
    dfS.loc[:df.index[-1], 'q1signal'] = q1signal

    lastsignal = predictionsY[-1]
    dfS.loc[:df.index[-1], 'lastsignal'] = lastsignal
    # dfS.loc[:df.index[-1], 'q1signalRolling'] = (dfS['lastsignal'] - 0.000001).rolling(window = 7).quantile(0.25).fillna(9999)
    dfS.loc[:df.index[-1],'q1signalRolling']=(dfS.loc[:df.index[-1],'lastsignal']-0.000001).rolling(window=7).quantile(0.25).fillna(9999)

    # todo:
    #  q1 signal < rolling signal = 0.75
    # if signal > 0

    dfS.loc[:df.index[-1], 'position'] = np.where(dfS.loc[:df.index[-1], 'q1signalRolling']== 9999, 0.0, dfS.loc[:df.index[-1], 'position'])

    if dfS.loc[df.index[-1], 'q1signalRolling']== 9999:
        dfS.loc[df.index[-1], 'position']== 9999

    else:

        if (lastsignal > 0.0001) & (lastsignal > q1signal) & (lastsignal < 0.001):
            dfS.loc[df.index[-1], 'position'] = 1.0

        elif 0.001 > lastsignal > 0.0001:
            dfS.loc[df.index[-1], 'position'] = 0.75

        elif (lastsignal > 0.001) & (dfS.loc[df.index[-1], 'q1signalRolling'] < lastsignal) & (dfS.loc[df.index[-1], 'q1signal'] < dfS.loc[df.index[-1], 'q1signalRolling']):
            dfS.loc[df.index[-1], 'position'] = -1.0

        else:
            dfS.loc[df.index[-1], 'position'] = 0.0
            # dfS.loc[:df.index[-1], 'position'] = 0


    print(' ')
    print('Last signal:\t%.6f' % dfS.loc[df.index[-1], 'lastsignal'])
    print('Q1 signal:\t\t %.6f' % dfS.loc[df.index[-1], 'q1signal'])
    print('Rolling signal:\t %.6f' % dfS.loc[df.index[-1], 'q1signalRolling'])
    print('\nCurrent Position:\t %.2f' % dfS.loc[df.index[-1], 'position'])

    featureImportances = pd.DataFrame(dfFeat)

    dfS.loc[:df.index[-1],'rtnPort'] = dfS.loc[:df.index[-1], 'position'] * dfS.loc[:df.index[-1], 'rtnSPY']
    # calculate cumulative portfolio return
    dfS.loc[:df.index[-1], 'cRtn-Port'] = (1 + dfS.loc[:df.index[-1], 'rtnPort']).cumprod()-1
    print('Cumulative Return To Date:\t %.2f' % dfS.loc[df.index[-1], 'cRtn-Port'])
...
if __name__ == '__main__':
    ...
    try:
        ...
        dfS.index=dfS.index.to_frame()[0]
        dfS.index.name='date'
        ...
        dfU['rtnSPY'] = dfU['Adj_Close'].pct_change().shift(-1)
        dfU.index = pd.DatetimeIndex(dfU.index)
        dfS = dfS.merge(dfU['rtnSPY'], how = 'left', left_index = True, right_index = True)
    
        # calculate cumulative asset return
        dfS['cRtn-SPY'] = ((1 + dfS['rtnSPY']).cumprod() - 1).fillna(0)
