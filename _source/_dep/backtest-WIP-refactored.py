...
def fnAggActivityFeed(df1, df2):
    ...
    dfAgg.drop(columns=['simple-rtnYesterdayToToday', 'simple-rtnPriorTwoDays', 'simple-rtnYesterdayToTodayClassified',], inplace=True)
...
def firstQuantileRisk(value, q1signal, posBins = [1.0, 0.75, 2.0]):
    result = np.where(value > 0,
                      np.where(value > q1signal, posBins[0], posBins[1]),
                      posBins[2])
...
def fnCreatePositions(predictionsY, signal):
    ...
    signal.loc[index_y, 'q1signalRolling'] = signal['q1signal'].expanding(min_periods=1).quantile(0.25) - 0.000001
    ...
    riskToSpy = (len(predictionsY) - 1) / sum([firstQuantileRisk(n, signal.loc[index_y]['q1signalRolling'][-1]) for n in predictionsY])
    # riskToSpy = (len(predictionsY) - 1) / sum([firstQuantileRisk(n, signal.loc[index_y, 'q1signal']) for n in predictionsY])


    if ((lastsignal > signal.loc[index_y]['q1signalRolling'][-1]) & (lastsignal > 0.000001)):
        return index_y, riskToSpy,signal.loc[index_y]['q1signalRolling'][-1]

    elif (lastsignal > 0.000001):
        return index_y, riskToSpy * 0.75,  signal.loc[index_y]['q1signalRolling'][-1]

    elif (lastsignal > 0) & (lastsignal < 0.000001):
        return index_y, 0.0, signal.loc[index_y]['q1signalRolling'][-1]

    else:
        return index_y, -1.0, signal.loc[index_y]['q1signalRolling'][-1]
...
def fnPositionBinning(df, posBins=[1.0, 0.75, -1.0]):
    ...
    df['position'] = np.where(df.signal > 0,                                                           # if signal > 0
                              ...
                              np.where(df.signal > (df.signal.quantile(0.25)- 0.000001),      # if signal greater than rolling median - 0.000001
