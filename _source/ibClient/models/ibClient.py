...
class BaseModel(object):
	...
	def account_value(self):
		dfAccnt = pd.DataFrame(self.ib.accountValues()).groupby('tag')['value'].sum()
		return dfAccnt
...
class HftModel1(BaseModel):
	...
	def run(self, ticker_tk=None, position=0):
		""" Entry point """
	...
	def perform_trade_logic(self):
		...
		if self.is_position_flat:
			...
			# self.logging.info_strategy_params()
	...
	def print_account(self):
		...
		# todo: logging.info current positon / ticker price info
		# position=100.0, marketPrice=366.14001465, marketValue=36614.0, averageCost=303.491603, unrealizedPNL=6264.84, realizedPNL=0.0
		# logging.info(self.request_portfolio()[0][1:-1])



	# def logging.info_strategy_params(self):
	# 	logging.info('[{time}][strategy params]beta:{beta:.2f} volatility:{vr:.2f}|rpnl={rpnl:.2f}'.format(
	...
	def place_spread_order(self, qty):
		...
		logging.info('Order placed: {}'.format(trade_a))
...
#
		...
		#
		# logging.info('[{time}][account]{symbol_a} pos={pos_a} avgPrice={avg_price_a}|'
