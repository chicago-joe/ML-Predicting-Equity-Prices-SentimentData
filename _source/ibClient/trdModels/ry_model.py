...
from util.ib_util import IBUtil
from util.contract_data import ContractData
...
class HFTModel:
    ...
    def __is_overbought_or_oversold(self):
        ...
        if (((self.prices[symbol_a].values[0] - self.statistics_params.m *
              self.prices[symbol_b].values[0] - self.statistics_params.b) <=
             (2 * self.statistics_params.std * self.statistics_params.muti + self.statistics_params.avg)) and
                ((self.prices[symbol_a].values[0] - self.statistics_params.m *
                  self.prices[symbol_b].values[0] - self.statistics_params.b) >=
                 -(2 * self.statistics_params.std * self.statistics_params.muti + self.statistics_params.avg))):

            # Buy BP-m * RSDA-b > 1*std + 0.5
            if ((self.self.prices[symbol_a].values[0] - self.statistics_params.m * self.self.prices[symbol_b].values[
                0] - self.statistics_params.b) > (self.statistics_params.std * self.statistics_params.muti + self.statistics_params.avg)):
                self.log('SELL PORTFOLIO,SELL{},BUY{}'.format(self.self.prices[symbol_a].values[0], self.self.prices[symbol_b].values[0]))
                # Keep track of the created order to avoid a 2nd order
                self.order = self.sell(self.datas[0], size = self.statistics_params.size)
                self.order = self.buy(self.datas[1], size = int(self.statistics_params.m * self.statistics_params.size))

            # BP-m*RSDA-b <-(1*std + 0.5)
            elif ((self.self.prices[symbol_a].values[0] - self.statistics_params.m * self.self.prices[symbol_b].values[
                0] - self.statistics_params.b) < -(self.statistics_params.std * self.statistics_params.muti + self.statistics_params.avg)):
                self.log('BUY PORTFOLIO,BUY{},SELL{}'.format(self.self.prices[symbol_a].values[0], self.self.prices[symbol_b].values[0]))
                # Keep track of the created order to avoid a 2nd order
                self.order = self.buy(self.datas[0], size = self.statistics_params.size)
                self.order = self.sell(self.datas[1], size = int(self.statistics_params.m * self.statistics_params.size))
        ...
        # this part is not exactly what I want but till now it is fine
            ...
            if ((self.self.prices[symbol_a].values[0] - self.statistics_params.m * self.self.prices[symbol_b].values[
                0] - self.statistics_params.b) >= (
                    2 * self.statistics_params.std * self.statistics_params.muti + self.statistics_params.avg)):
                self.log('CLOSE THE SELL LIMIT LOSS,BUY{},SELL{}'.format(self.self.prices[symbol_a].values[0],
            ...
            # if the price is too low we need to sell our portfolio back incase of risk(sell BP buy RDSA)
                ...
                0] - self.statistics_params.b) <= -(
                    2 * self.statistics_params.std * self.statistics_params.muti + self.statistics_params.avg)):
                self.log('CLOSE THE BUY LIMIT LOSS,SELL{},BUY{}'.format(self.self.prices[symbol_a].values[0],
                                                                        self.self.prices[symbol_b].values[0]))
                # Keep track of the created order to avoid a 2nd order
                self.order = self.sell(self.datas[0], size = self.statistics_params.size)
                self.order = self.buy(self.datas[1], size = int(self.statistics_params.m * self.statistics_params.size))
                ...
                # -0.5 < BP-m*RSDA-b < 0.5            
                if (((self.self.prices[symbol_a].values[0] - self.statistics_params.m * self.self.prices[symbol_b].values[
                    0] - self.statistics_params.b) < self.statistics_params.avg) and
                        ((self.self.prices[symbol_a].values[0] - self.statistics_params.m * self.self.prices[symbol_b].values[
                            0] - self.statistics_params.b) > -self.statistics_params.avg)):

                    # comes from higher portfolio value
                    if ((self.self.prices[symbol_a].values[0] - self.statistics_params.m * self.self.prices[symbol_b].values[
                        0] - self.statistics_params.b) > 0):
                        self.log('CLOSE THE SELL,BUY{},SELL{}'.format(self.self.prices[symbol_a].values[0],
                                                                      self.self.prices[symbol_b].values[0]))
                        # Keep track of the created order to avoid a 2nd order
                        self.order = self.buy(self.datas[0], size = self.statistics_params.size)
                        self.order = self.sell(self.datas[1], size = int(self.statistics_params.m * self.statistics_params.size))
                    ...
                    # comes from lower portfolio value
                        ...
                        self.log(
                            'CLOSE THE BUY,SELL{},BUY{}'.format(self.self.prices[symbol_a].values[0], self.self.prices[symbol_b].values[0]))
                        # Keep track of the created order to avoid a 2nd order
                        self.order = self.sell(self.datas[0], size = self.statistics_params.size)
                        self.order = self.buy(self.datas[1], size = int(self.statistics_params.m * self.statistics_params.size))
