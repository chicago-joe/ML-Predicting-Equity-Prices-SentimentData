...
class DTLearner(object):
    ...
    def addEvidence(self,dataX,dataY):
        ...
        if self.verbose: print('Building the Decision Tree')
        self.tree = self.__build_tree__(data)

    def __build_tree__(self, data):
        """
        @summary: Build a decision tree using JR Quinlan's algorithm
        @param dataX: X values of data to add
        @param dataY: the Y training values
        ...
        # Check if all predictions are the same
            ...
            best_feature = self.__best_feature__(dataX, dataY)
            # split_value = np.median(dataX[:,best_feature])
            split_value = self.__choose_best_split__(dataX, best_feature)
            left_tree = self.__build_tree__(data[data[:,best_feature] <= split_value])
            right_tree = self.__build_tree__(data[data[:,best_feature] > split_value])
    ...
    def __best_feature__(self, dataX, dataY):
        """
        @summary: calculates the best feature by correlation metric
        @param dataX: X values of data to add
        @param dataY: the Y training values
        @return: returns index of best feature
        """
        ...
        for feature in range(0,dataX.shape[1]):
            ...
            corr = np.corrcoef(dataX[:,feature], y=dataY)
            a_corr = abs(corr[0,1])
            if self.verbose: print('Correlation : {}'.format(a_corr))

            if a_corr >= max_corr:
                max_corr = a_corr
                max_feature = feature

        if self.verbose: print('Best feature is {} with correlation {}'.format(max_feature, max_corr))

        return int(max_feature)

    def __choose_best_split__(self, dataX, best_feature):
        """
        @summary: choose best split value if split value leads to imbalanced tree
        @param dataX: X values of data to add
        @param best_feature: index of best feature
        @return: split value for balanced tree
        """

        split_val = np.median(dataX[:,best_feature])

        left_data = dataX[dataX[:,best_feature] <= split_val]
        right_data = dataX[dataX[:,best_feature] > split_val]

        while left_data.shape[0] == 0 or right_data.shape[0] == 0:

            if self.verbose: print('Hit edge case')
